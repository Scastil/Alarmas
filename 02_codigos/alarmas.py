...
def plotN_vs_History(est,start,end,dfconfig,n_pronos,rutaN,rutafigsN,rng1,timedeltaEv):

            #Consulta ultimas 3 horas.
            self= cpr.Nivel(est)
            level=self.get_level(start,end)
            level=level['nivel']
            #si la media de los ultimos 5 minutos es nan- no hay datos- : no hago nada.
            if np.isnan(level[-5:].mean()):
                pass
            else:
                #resample
